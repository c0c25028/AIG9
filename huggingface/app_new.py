...
class Config:
    ...
    MAX_HISTORY_LENGTH = 10  # 保持する会話履歴の最大数
...
class ConversationCache:
    ...
    def __init__(self, max_length: int = Config.MAX_HISTORY_LENGTH):
        self.history: List[ConversationEntry] = []
        self.max_length = max_length
    
    def add_entry(self, user_question: str, ai_answer: str):
        """会話エントリを追加"""
    ...
    def get_formatted_history(self) -> str:
        ...
        for i, entry in enumerate(self.history, 1):
            formatted.append(f"[{entry.timestamp}] ユーザー: {entry.user_question}")
            formatted.append(f"AI: {entry.ai_answer[:200]}{'...' if len(entry.ai_answer) > 200 else ''}")
            formatted.append("")
    ...
    def get_display_history(self) -> str:
        ...
        for entry in self.history:
            ...
            formatted.append(f"**👤 ユーザー:** {entry.user_question}")
            formatted.append(f"**🤖 AI:** {entry.ai_answer[:300]}{'...' if len(entry.ai_answer) > 300 else ''}")
            formatted.append("---")
...
class GeminiHandler:
    ...
    def generate_response(self, user_question: str, conversation_history: str = "",
                         previous_steps: Optional[List[ParsedStep]] = None, 
...
class AISystem:
    ...
    def process_question_iterations(self, user_question: str, progress_fn):
        ...
        # 会話履歴を取得
        conversation_history = self.conversation_cache.get_formatted_history()
        
        # 最初のLLM呼び出し
        for update in self.llm_handler.generate_response(user_question, conversation_history):
            if "ERROR:" in update:
                raw_output = update
                break
            if update.endswith("..."):
        ...
        for i in range(Config.MAX_SEARCH_ITERATIONS):
            ...
            for update in self.llm_handler.generate_response(user_question, conversation_history, steps, search_results):
                if "ERROR:" in update:
                    raw_output = update
                    break
                if update.endswith("..."):
        ...
        # 会話履歴に追加
        self.conversation_cache.add_entry(user_question, answer)
        ...
        yield (answer, graph_image, error_html, self.conversation_cache.get_display_history())
...
def create_gradio_interface():
    ...
    with gr.Blocks(theme=gr.themes.Soft(primary_hue="blue")) as demo:
        ...
        gr.Markdown("# 🧠 AI思考連鎖可視化システム with 会話履歴")
        gr.Markdown("""
        Gemini APIを利用して、AIの思考プロセスを段階的に可視化します。
        
        **✨ 新機能:** 会話履歴を保持し、文脈を考慮した対話が可能です。
        - 💾 最大10件の会話を自動保存
        - 🔄 過去の質問・回答を参照した対話
        - 🗑️ いつでも履歴をクリア可能
        """)
        
        with gr.Accordion("APIキー設定", open=not has_all_keys, visible=not has_all_keys):
            with gr.Row():
        ...
        with gr.Accordion("システムプロンプト設定", open=False):
            system_prompt_input = gr.Textbox(
                ...
                info="プロンプト内で {user_question}, {conversation_history}, {previous_steps_str}, {search_results_str} を使用できます"
            )
        ...
        with gr.Column(visible=has_all_keys) as main_interface:
            ...
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    with gr.Accordion("📜 会話履歴", open=True) as history_accordion:
                        with gr.Row():
                            gr.Markdown("### 保存された会話")
                            clear_history_button = gr.Button("🗑️ 履歴クリア", variant="stop", size="sm")
                        history_display = gr.Markdown(
                            value="会話履歴はまだありません。",
                            elem_classes="history-box"
                        )
                        history_status = gr.Markdown(value="", visible=True)
            ...
            gr.Examples(
                examples=[
                    ...
                    ["さっきの話についてもう少し詳しく説明してください。（※会話履歴を利用した質問例）"]
                ], 
        ...
        def clear_history(system):
            ...
            if system is None:
                return (
                    gr.update(value="会話履歴はまだありません。"),
                    gr.update(value="<p style='color:orange;'>⚠️ システムが初期化されていません。</p>"),
                    gr.update(value=""),
                    gr.update(value=None),
                    gr.update(value="")
                )
            
            count = system.conversation_cache.get_count()
            system.conversation_cache.clear()
            
            return (
                gr.update(value="会話履歴はまだありません。"),
                gr.update(value=f"<p style='color:green;'>✅ 会話履歴をクリアしました ({count}件削除)</p>"),
                gr.update(value=""),
                gr.update(value=None),
                gr.update(value="")
            )
        ...
        set_api_key_button.click(
            ...
            outputs=[system_state, api_status_output, main_interface, history_display, history_status]
        )
        ...
        submit_button.click(
            ...
            outputs=[answer_output, graph_output, graph_errors_output, history_display, history_status]
        )
        ...
        clear_history_button.click(
            fn=clear_history,
            inputs=[system_state],
            outputs=[history_display, history_status, answer_output, graph_output, graph_errors_output]
        )
        
        # 環境変数にキーがある場合は起動時に自動設定
        if has_all_keys:
            def auto_init():
                system = AISystem(
